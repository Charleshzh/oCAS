...
## 0.27.3 后续调研：1892 题进展为何低（2026-09-12）

> 本节是**归因调研**，不是一次波次验收。所有数字来自 0.27.3 终态产物
> （`failures_final_0273.jsonl`、`report_final_0273.json`）与 Rubi 官方测试档案里
> **参考答案**的对照，以及可复现的单例探针（`OCAS_INTEGRATE_TRACE=1` 与 release
> bench 的 `--case` 子模式）。凡属估算的口径都在文中标明。

### 1. 结论摘要

1. **剩余 1522 题几乎没有「差一口气」的**：只有 **11 题（0.7%）** 与某个**已解**题目
   共享同一「形状骨架」（参数改名为 `@`、数字改名为 `#` 后的结构）——即 98% 以上的
   失败需要 oCAS 从未具备的能力，而不是把现有机制调松一点。
2. **81.9% 的失败其参考答案是纯初等的**，问题出在初等机制，不在特殊函数。
3. 已定位**两个可量化的架构性缺陷**（下面第 3 节），其中「残项未解析」一项已实测出
   **净 +4 题**（+5 新解 / −1 回归 / +3 新超时）。
4. 还有**结构性天花板**（第 4 节）：缺二重对数（8.0% 的题参考答案需要 `polylog`）、
   单根式基假设、无代数扩张、无反函数代换引擎、符号参数入口闸门。
5. **趋势是收益递减**：+6.82pp（0.27.1）→ +2.01pp（0.27.2）→ +1.11pp（0.27.3），
   而 0.27.0 设定的验收线是 **+30pp**（目标 35.87%，当前 19.56%，缺口 16.31pp）。

### 2. 按「参考答案可达性」分类（1892 题子集，失败 1522 题）

对每一道失败题取 Rubi 档案里的**期望原函数**，按它需要什么来分类：

| # | 类别 | 题数 | 占比 | 含义 |
|---|---|---|---|---|
| 0 | Rubi 自己给 `Unintegrable` | **75** | 4.9% | **参考实现自己放弃**，oCAS 回退是正确行为；这类题永远不可能「解出」 |
| 1 | 答案需要 `polylog`/`Li₂` | **122** | 8.0% | **今天不可达**：oCAS 连这个函数头都没有，机制再宽也无用 |
| 2 | 答案用到 oCAS **已有**的特殊头（erf/Si/Ci/Ei/Fresnel/椭圆） | **79** | 5.2% | 可达，缺的是更难的复合形状 |
| 3 | 答案**纯初等** | **1246** | 81.9% | **真正的主战场**：初等积分但现有初等机制打不到 |

口径修正：把第 0 类剔除后，「参考可解」的分母是 1447，当前对它的覆盖率是
**370/(370+1447) = 20.4%**；`+30pp` 验收线在这种口径下要求的是**机制级**的跃迁。

（估算）第 3 类 1246 题内部再用粗糙正则分桶：含 **≥2 个不同根式基** 约 222 题、
自由参数 **≥6** 约 97 题（超出 `symbolic_rational` 的 5 符号入口闸门）、其余含单一
根式基或纯有理。这些子桶的边界受正则限制，仅作量级参考。

### 3. 两个已定位、可量化的架构性缺陷

#### 3.1 残项未解析（`rational` / `symbolic_rational` 返回「带残项的成功」）

- **现象（最小复现）**：`1/(x²+5)` **解得**；把它写成分解后的和
  `(1/5)/x² − (1/5)/(x²+5)` 也**解得**；但 `1/(x²(x²+5))` **回退**，输出是
  `… + Integral(−1/5·(x²+5)⁻¹, x)` —— 分式分解**已经算出来了**，剩下的那一片
  明明是同一个可解积分，却被原样留在结果里。
- **根因**：`rational.rs::integral_fallback`（第 629–638 行）与
  `symbolic_rational.rs`（第 1250–1257 行，分母因子次数 > 2 的「诚实部分结果」）
  都会构造 `Integral(...)` 节点并作为 `Some(...)` 返回；而 `rules.rs::integrate_rules`
  会在返回前调用 `resolve_residuals`，**这两处不会**，于是管线把它当成最终回退。
- **实测**（原型：在这两处加 `contains_integral` → `resolve_residuals`）：
  | 指标 | 0.27.3 终态 | 加原型后 |
  |---|---|---|
  | solved | 370 | **374** |
  | 逐题 diff | — | **新解 5、回归 1** |
  ...
  | 墙钟 | 461.2 s | 464.6 s |
  新解：`rubi-00179`、`00543`、`00627`、`00638`、`01798`（`1/(x^6(5+x^2))` 正是探针
  阶梯里那一例）；回归：`rubi-01646`（`sin x/(−2+cos x+cos²x)`）；新超时：
  `rubi-00136`、`00331`、`01377`（tan 有理式与 `1/(a+b·cosh+c·cosh²)`）。
- **结论与建议**：缺陷真实、收益真实（净 +4），但**朴素的「立刻解析」会抢路并引入新超时**。
  正确做法是**把残项解析放到链尾**（先让所有既有阶段按原顺序尝试，链末再对残留的
  `Integral(...)` 做一次**带确定性预算**的解析），并把嵌套解析纳入工作量/链条目预算。
  原型已**回退**，仅保留本段测量（产物：`bench_0273_residuefix.log`、
  `failures_residuefix.jsonl`）。

#### 3.2 全局链条目预算静默截断（`MAX_CHAIN_ENTRIES = 256`）

- **现象**：`x^6/(216+108x²+324x³+18x⁴+x⁶)²`（**零参数、纯有理**）整条 21 阶段链
  **重复了 18 轮、共 306 次阶段进入**，越过 256 的上限后，`try_risch_or_fallback`
  对之后每一次进入都**立即返回 fallback**——包括本该解析的子积分。
- **影响面**：该 backstop 是为 0.27.0 的 `parts ↔ Weierstrass` 乒乓死循环加的；
  但它按「每顶层调用 256 次」计量，**合法但会重入的工作会被误杀**，同时白烧墙钟
  （18 轮 × 21 阶段）。
- **建议**：把「重复进入同一表达式」的循环检测做在**表达式层面**（同一形状第二次进入
  即拒绝），而不是给所有工作一个总量上限；或在预算耗尽时**先解析已有残项再退出**。

补充：`1/(1+x⁴)`、`1/(1−3x²+x⁴)`、`(1+x²)/(1+4x²+x⁴)`、`1/(1−x⁴+x⁸)`、
`1/(x⁴+1)²` 与上面的不可约六次式全部回退——**根因是分母在 ℚ 上不可约/需要代数扩张**，
SymPy 对这些题本身就返回 `RootSum` 或带 `√` 的形式。`symbolic_rational` 的
`split_squarefree_factors` 在因子次数 > 2 时直接放弃（第 1249–1257 行），即
**没有代数扩张上的留数计算**。这是算法缺口，不是 bug。

### 4. 结构性天花板（机制路线 vs 规则路线）

| 维度 | 现状 | 量化 |
|---|---|---|
| 题库性质 | Rubi 官方 Axiom 回归套件（72,944 题，7000+ 手写规则） | 题库**奖励规则模式广度**，机制路线天然吃亏 |
| 缺失函数种类 | 无 `polylog`/`Li₂`（也无 productlog/gamma/zeta 的积分归约） | **122 题（8.0%）**在今天的库上不可达 |
| 根式基 | 每个根式机制只认**单一基** | 估算 **222 题**含 ≥2 个不同基（`√(cos u)` 与 `√(a+b cos u)` 并存等） |
| 代数扩张 | 无 | 不可约三次/四次/六次/八次分母的整族回退 |
| 反函数代换 | `inverse_trig.rs` 只覆盖核导数幂/线性变元，无「多项式权 × `(a+b·f(ax+b))^k`」引擎 | `inverse-trig-hyper` 桶 **138/146 未解（94.5%）** |
| 对数分母/对数幂 | 无 dilog 产出的归约 | `exp-log` 桶 **79/83 未解（95.2%）**，其中 26 题参考答案含 `polylog` |
| 符号参数 | `symbolic_rational` 入口闸门 5 符号 | **169 题**自由参数 ≥6（估算 97 题落在纯初等主战场） |
| 最差/最好桶 | inverse-trig-hyper 94.5%、exp-log 95.2%、mixed-other 88.0%、radical 87.5% 未解 ↔ power-binomial 54.3%、special 21.1% | 桶间差异极大：**机制命中率取决于题库中该族的规模** |

### 5. 趋势与验收线

| 版本 | 覆盖率 | 边际 | 新增机制量级 |
|---|---|---|---|
| 0.26 链基线（Risch 路线） | 5.87% | — | — |
| 0.27.0(c) | 9.62% | +3.75pp | 规则表 + 展开重试 + 积化和差 |
| 0.27.1 | 16.44% | **+6.82pp** | 7 个机制模块 |
| 0.27.2 | 18.45% | +2.01pp | 8 项机制 + 预算治理 |
| 0.27.3 | 19.56% | **+1.11pp** | 4 个特殊函数归约族 + 精确折叠 + 仿射半幂 |

边际收益从 +6.82pp 降到 +1.11pp（约 6 倍衰减），而**每个机制模块的覆盖增量已进入
「个位数题」区间**（0.27.3：special 族 +15、半幂仿射 +4、折叠 +2）。这符合第 1 节
的结论：容易的族已被吃光，剩下的要么是架构缺陷（3.1/3.2，合计可回收约 4–10 题）、
要么是算法天花板（第 4 节，每一族需要**新引擎**）、要么根本不可达（`polylog` 与
`Unintegrable` 合计 197 题，13%）。

### 6. 给 0.28.0–0.32.0 的优先级建议（按「可回收题数 ÷ 风险」排序）

1. **修 3.1 残项解析（链尾 + 预算）**：预期 +4～+10，风险中（须避免原型里的 1 回归
   3 超时）。**不需要新引擎**，杠杆最高。
2. **修 3.2 循环检测改为表达式级**：直接改善墙钟与超时，并解开被误杀的合法重入。
3. **单根式基 → 多根式基归约**：估算 222 题量级，但需要新引擎（0.27.3 的
   `halfpower` 只做到仿射变元层，140 例仿射 `cos` 半幂簇仍 136 例拒绝）。
4. **代数扩张上的留数（不可约二次以上因子）**：覆盖一整个家族（`1/(1+x⁴)` 类），
   需要 Lazard–Rioboo–Trager 的代数数留数实现。
5. **`polylog`/`Li₂` 头 + 对数积分归约**：先补函数头与数值核验，再补归约；
   122 题（8.0%）在此解锁。
6. **反函数代换引擎**：`inverse-trig-hyper` 94.5% 未解，是最大的单一未触碰桶。

---

## 0.27.3 符号积分广度（Rubi 1892 题子集，2026-09-12）

> 本轮按《0.27.3 波次计划》执行：特殊函数导数表 + 数值 oracle 扩头、`special.rs` 四个
> 特殊函数归约族、精确线性平方折叠（仿射底）、半幂前端仿射变元 + 两支片层因子。
> 两个机制族由子智能体按「单文件所有权」并行完成（`special.rs` / `halfpower.rs`），
> 主线负责 `derivative.rs`、`integral_eval.rs`、`mod.rs` 前置折叠、口径与验收。

| 口径 | solved | fallback | **verified** | mismatches | 超时 | 崩溃 | 总墙钟 |
|---|---|---|---|---|---|---|---|
| 0.27.2 终态（基线） | 349（18.45%） | 1543 | 325 / 349（93.1%） | 0 | 13 | 0 | 445.7 s |
| **0.27.3 终态（2026-09-12）** | **370（19.56%）** | **1522** | **343 / 370（92.7%）** | **0** | **12** | **0** | **461.2 s** |

- 逐题 diff（`diff_1892_failures.py`）：**新解 21 例、回归 0 例**。桶 delta：
  special +15、trig +3、power-binomial +2、mixed-other +1。
  新解 21 例：`rubi-00123`、`00146`、`00276`、`00291`、`00377`、`00445`、`00575`、
  `00587`、`00854`、`00953`、`01009`、`01052`、`01112`、`01172`、`01267`、`01516`、
  `01598`、`01633`、`01729`、`01763`、`01799`。
- 机制归因：special +15 全部来自 0.27.3 的四个归约族（**15 例全部通过数值验证**）；
  trig +3 / mixed-other +1 来自半幂前端仿射变元（4 例，其中 4 例在赛题 oracle 判为
  inconclusive，因为其发射参数固定为 `m=2`，超出 oracle 的 `|m| ≤ 0.9` 窗口——
  `halfpower` 模块自带的独立数值 oracle 在 1e-9 上核验通过，这不是错案）；
  power-binomial +2 来自精确线性平方折叠（`rubi-00146`、`rubi-00854`）。
- **`rubi-00854` 的根因修复**：`(a + b*x)/((d + e*x)^4*(a^2 + 2*a*b*x + b^2*x^2))`
  在 0.27.2 是 10 s 单题超时；折叠把展开的三项式还原为 `(a + b*x)^2` 后，
  0.27.2 的 `symbolic_rational` 后端在 **0.01 s** 内求解（单元测试钉死）。折叠只对
  **底对积分变量仿射**的三项式生效、且必须通过「重新展开复现原和」验证；非整数幂之下
  一律不折叠（`((p+q)²)^{1/2} = |p+q| ≠ p+q`）。这一限制是被逐题 diff 逼出来的：
  早期版本会把 `a²cos²u + 2ab·cos u·sin u + b²sin²u` 折回 `(a·cos u + b·sin u)²`，
  从而撤掉积化和差阶段需要的展开形，使 `rubi-00334` 由「已解」变为 10 s 超时
  （interim 全量运行捕获，`+3 → 0` 回归）。
- 超时 13 → 12（净 −1）。**目标 ≤5 未达**。12 例残存超时已逐例阶段归因
  （`OCAS_INTEGRATE_TRACE=1` 取最后一条 `[trace] enter`，落档
  `ocas-tests/data/timeout_attribution_0273.csv`）：**`symbolic_rational` 9 例**
  （`rubi-00166`、`00272`、`00644`、`00834`、`01005`、`01336`、`01672`、`01707`、`01824`）、
  `binomial` 1（`00513`）、`rational` 1（`01344`）、`trig_kernel` 1（`01014`）。
  其中 `00834` 的轨迹显示它已由 `binomial` 的 `t = √(d+e·x)` 有理化成功，
  挂在后续对 `t²/((a + b·(−d + t²)/e)²)` 的符号有理积分上——**结论：0.28.0 应在
  `symbolic_rational` 上加确定性工作量预算/预empt，而不是再加墙钟**（0.27.2 的同一教训）。
- **诚实记录：四项验收线未达**——solved 目标 ≥371（差 1）、已验证比例目标 ≥95%
  （92.7%）、超时目标 ≤5（12）、墙钟目标 <445.7 s（461.2 s，+3.5%）。墙钟上升主要来自
  新增归约族在 `special` 阶段的形状扫描与折叠的前置遍历；折叠已加「`normalize` 后
  `2·p·q` 快速通道」，把该部分从 482 s 降到 461.2 s。
- **撤回记录（同样重要）**：本轮实现过「线性分式对数归约」
  （`C·(A + B·log Q)·(F + G·x)^p` 分部，`integral/log_fraction.rs`），在 interim 全量
  运行中被数值 oracle 判为**真错案**：残项是裸乘积而不是 `Integral(...)` 原子，
  `resolve_residuals` 原样返回，于是结果「无残项但错误」——正是 0.27.2 引入双口径时
  要防的那类。修正残项包装后，其残项仍无法被有理后端可靠积分（留下残项或产出巨大表达式），
  且 debug 测试代价 163 s。按「只保留无回归且无墙钟代价的代码」原则**整体撤回**，
  文件已删除。该尝试的副产品是 A4 回归护栏升级：由「必须留残项」改为「若求解则数值求导
  核验」，两种结局都不放过错案。
- 未验证 27 例（全部 inconclusive，非错案）：定义域受限头、含虚数单位 `i`、
  步长自洽闸门判不可判，以及 4 例半幂仿射变元的 `|m| ≤ 0.9` 窗口外样本。
- **0.27 线冻结判定**：0.27.2（+38）与 0.27.3（+21）**连续两波各自净增 < 60**，
  按 `EVOLUTION_PLAN` 字面规则**冻结 0.27 线，下一活动线为 0.28.0**；
  0.27.3 为 0.27 系列最后一版。
- 复现命令：`cargo bench -p ocas-tests --bench integrate_1892`；
  逐题 diff：`uv run python ocas-tests/scripts/diff_1892_failures.py
  ocas-tests/data/failures_final_0272.jsonl ocas-tests/data/failures_final_0273.jsonl`。
  终态产物：`failures_final_0273.jsonl`、`report_final_0273.json`、
  `unverified_final_0273.jsonl`、`bench_final_0273.log`。

---

## 0.27.2 符号积分广度（Rubi 1892 题子集，2026-09-11）

> 本轮按《0.27.2 波次计划》执行：阶段打点 + 悬挂归因、有界展开预通道、五个阶段的
> 确定性工作量预算、**独立数值验证 oracle（已验证覆盖率）**、双曲闭式族、有理导数核
> 代换、三角相位归一、逆函数复合消去、`exp(逆函数)` 代数化、半幂前端 + 椭圆
> Legendre 约化，以及六组错案修复。机制实现由子智能体按文件分工完成，主线负责
> 集成、口径与验收。

| 口径 | solved | fallback | **verified** | mismatches | 超时 | 崩溃 | 总墙钟 |
|---|---|---|---|---|---|---|---|
| 0.27.1 终态（基线） | 311 | 1581 | 未度量 | 未度量 | 33 | 0 | 572.0 s |
| **0.27.2 终态（2026-09-11）** | **349（18.45%）** | **1543** | **325 / 349（93.1%）** | **0** | **13** | **0** | **445.7 s** |

- 逐题 diff（`diff_1892_failures.py`）：**新解 44 例、主动回退 6 例，净 +38**。
  新解 44 例中 **32 例通过数值验证**，12 例判为 inconclusive（定义域受限或含
  `i`，非错案——本轮 mismatches = 0）。6 例主动回退全部是 0.27.1 的**错案**
  （在本次早期全量运行中逐个被 oracle 判为 mismatch），修复方式选择「诚实回退」
  而非继续输出错误闭式：
  `rubi-00367`/`rubi-01215`（`symbolic_rational` 重因子留数系数错误）、
  `rubi-00531`/`rubi-01482`/`rubi-01662`（Risch `integrate_kpoly_primitive`
  把非域项留在顶层，例如 `∫log(1+x²)/(1+x²)` 曾返回 `atan(x)`）、
  `rubi-01135`（规则 A4 闭包的序列通配符吞掉 x 相关因子）。
  `symbolic_rational` 另新增**发射前数值自检**：组装结果若在采样点上与输入
  不符即回退，从机制上杜绝该类错案再次流出。
- 桶 delta（对新解）：mixed-other +30、trig +7、power-binomial +4、hyperbolic +2、
  radical +1。
- 超时：33 → 13（墙钟 −22%）。剩余 13 例族：双曲有理式（`sinh/cosh/coth` 线性分母
  的高次幂与多项式分子）、`P(x)·cos/sin(c+d·x)/Q(sin)^n`、`√(d+e·x)` 与
  `(c+d·x)^(1/2)` 根式、符号指数对数核。它们仍消耗每例 10 s 预算，是下一波的
  首要目标（闭式机制预empt，而非再加预算）。
- **已验证口径**：`ocas-tests/src/integral_eval.rs` 用 f64 求值 + 两档步长自洽的
  5 点中心差分 + 三轮确定性参数（混合符号、全正小值、全正大值）独立判断
  `d/dx(结果)` 是否等于被积函数；`erf/erfc/erfi` 用级数，`EllipticF/E/Pi` 用
  定义积分的自适应 Simpson。harness 报告 `verified_solved` /
  `unverified_solved` / `verify_indeterminate` / `verify_mismatches`，并分别转储。
  **mismatches = 0**：本轮没有任何"字符串口径已解但实际错误"的题目。
- 未验证 24 例（全部 inconclusive，非错案）：11 例结果含受限定义域的头
  （`atanh/acosh/sqrt` 在采样区间外），10 例含虚数单位 `i`（实数 oracle 无法采样，
  机制内另有 ℂ 检查），3 例步长自洽性闸门判为不可判。
- 本轮发现并修复的 0.27.1 错案类（全部由 oracle 定位，SymPy 独立复核）：
  1. `rational_square_root` 把多项式**和**当作单项式平方（`4a²+4b² → 2a+2b`），
     二次分母在伪根处分拆并输出错误对数；
  2. `complete_square` 假定分母首一（Hermite 递推会交回 `1−x²`），
     `∫dx/(x²−1)²` 错误返回 `atan(x)/2`；
  3. Chebyshev 第 3 类回代丢分支符号（`(a+b·x²)^(9/2)/x^12` 在 `x<0` 给出负原函数）；
  4. Risch `integrate_kpoly_primitive` 把非域项留在顶层；
  5. 规则 A4 闭包序列通配符吞掉 x 相关因子；
  6. 共振积化和差输出**恒等于零的分母**（`1/(d+(−1)·d)`，所有参数取值下为 ∞）。
  这些修复共消除 30+ 个错案（含规则表零分母安全网与 `trig_reduce` 的同类项折叠）。
- 诚实性说明：0.27.1 的 311 solved 中至少有 8 例是错案（上述 6 例 + 已被机制
  修复的 2 例），因此 0.27.2 的 349 与 325 都已用数值判据认证；口径升级的意义
  正在于此——「已验证覆盖率」比字符串覆盖率更接近真实能力。
- 与 +30pp 验收线的关系：仍未达（+2.0pp）。0.27.x 按计划作为连续系列推进，
  下一波（0.27.3）目标为剩余超时族、复合壳层拆分、特殊函数族与椭圆族广度。
