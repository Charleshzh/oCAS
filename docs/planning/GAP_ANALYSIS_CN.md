...
| 优先级 | 缺口 | 现状 | 目标 | 理由 |
|---|---|---|---|---|
| **P0** | 符号积分广度（Rubi 规则集成或等效） | Risch + 启发式四技术 + 0.27 规则表引擎（A–H 族）+ 符号常数有理后端 + 展开重试 + 三角积化和差 + 0.27.1 七机制模块（Chebyshev/三角核/根式/反三角/分母幂/核代换）；1892 题 **16.44%**（311/1892，+6.82pp 对 0.27.0，0 回归；诚实记录，详见 BENCHMARK_RESULTS_CN.md 0.27.1 段） | 对标 symbolica-integrate 1892 题 | Symbolica 2.2 Rubi 7000+ 规则（72,944 题库）仍是最大功能缺口；剩余主干为椭圆族与高符号商式（BENCHMARK_RESULTS_CN.md 0.27.1 段量化） |
