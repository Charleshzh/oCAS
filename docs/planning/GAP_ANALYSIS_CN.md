...
| 算法领域 | oCAS 现状 | 成熟度 |
|---|---|---|
| 多项式因式分解 | `DenseUnivariatePolynomial` 上 ℤ 与 ℤ_p 的 `factor()`，以及 `SparseMultivariatePolynomial` 上任意多元 ℤ 与 ℤ_p 的 `factor()`（0.16.0 Wang EEZ：≥3 变量 + 首项系数预处理[常数 LC] + Zassenhaus）；代数数域缺失；非常数 LC 强加推迟 0.16.1 | 🟢 一元/二元/任意多元 |
...
| 7a | 非常数首项系数强加 + 多元稀疏化 | 🔴 0.16.1——模 p Hensel 强加 + 稀疏 Diophantine |
