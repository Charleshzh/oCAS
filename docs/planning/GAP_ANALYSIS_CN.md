...
| Gröbner 基 | 🟡 F4 真实线性代数完成（0.15.1）+ LM 索引 + 稀疏 echelon（0.15.2）；cyclic-6 ℤ₁₃ 9970 s → 3670 s（2.7×）；<5s 未达，需 F5 签名约简 | ✅ 工业级 |
...
| 6 | Gröbner 大规模性能（cyclic-6 ℤ_p < 5 s） | 🟡 0.15.2 完成（9970 s → 3670 s，2.7×）；<5 s 定界 0.19 F5 签名约简（阶段 B++） |
