...
This document describes the architecture of **oCAS**, an LGPL-3.0+ Rust
computer algebra system. For the Chinese edition, see
...
1. **Performance first**: zero-copy parsing, arena allocation, hash consing,
   SIMD, parallelization, and mature LGPL numerical backends.
2. **License clarity**: the default build uses only LGPL-compatible backends.
   GPL-only backends are isolated in `ocas-gpl`.
3. **Layered design**: each layer depends only on lower layers; language
   bindings sit at the top.
4. **Multi-language consistency**: Rust is the source of truth; Python and
   C/C++ are thin wrappers.
5. **Backend pluggability**: polynomial, matrix, and number backends can be
   swapped via traits.
...
- Read-only expression trees can be shared across threads via `Arc` or
...
- Categories:
  ...
  - `BackendError`
- The Rust API returns `Result<T, OcasError>`.
- The FFI layer provides an `ocas_error_t` out-pointer and C string message.
...
- Built with PyO3.
- Wraps Rust types as Python classes (`Expression`, `Polynomial`, `Domain`,
  `Matrix`).
- Python objects hold a reference to the Rust arena; freeing the Python object
  drops the arena.
...
The following backends can be linked directly into the LGPL-3.0+ core without
forcing GPL:
...
The default build keeps backends optional, ensuring it compiles even on systems
without GMP/MPFR/FLINT (e.g., Windows MSVC).
...
To enable the full LGPL backend, pass the features explicitly:
...
- **Backend specialization**: select the fastest backend for each domain and
  size.
- **SIMD**: use `std::simd` / `packed_simd` for dense polynomial evaluation and
  numerical kernels.
- **Parallelization**: `rayon` for map operations, matrix kernels, and Gröbner
  basis selection.
- **JIT**: Cranelift for repeated expression evaluation; LLVM for heavily
  optimized AOT kernels.
...
- How to expose algebraic numbers and interval arithmetic cleanly across all
  three language APIs.
- Whether to support Mathematica syntax natively or only as a compatibility
  shim.
