...
**目标**：在 0.12 有理函数栈与 0.13 Gröbner F4 之间，整合经过验证的第三方
Rust/FFI 库，填补功能空白，但不引入新的算法纵向。这是一个纯性能/基础设施
版本。大整数/精确数后端（`malachite`、`dashu` 等）被有意排除在本版本之外，
它们仍属于 0.11.2 计算加速基础设施的工作范畴。

**功能**

| 条目 | 库 | 许可证 | oCAS 落地位置 |
|---|---|---|---|
| ℤ_p 上稠密多项式 FFT 乘法 | `ark-poly` | MIT/Apache-2.0 | `ocas-poly::mul::ntt` |
| 稀疏多项式快速求值 | `fast_polynomial` | MIT | `ocas-eval::poly_eval`（f64 稠密路径） |
| F4 的稀疏 Macaulay 矩阵存储 | `sprs` | MIT/Apache-2.0 | `ocas-poly::groebner::matrix` |
| 数值求解器正确性验证 | `faer` | MIT | `ocas-tests::verify` 中的求解器一致性验证 |
| 数值求根验证 | `roots` | BSD-2-Clause | `ocas-tests::verify` 中的根隔离验证 |
| 数值积分验证 | `quadrature` | BSD-2-Clause | `ocas-tests::verify` 中的 `integrate` 验证 |
| 表达式求值的通用 SIMD 分派 | `pulp` | MIT | `ocas-eval::simd`（AVX2/AVX-512/NEON） |
| 有限域/环抽象参考 | `feanor-math` | MIT | 只读参考，不作为依赖 |

**性能指标**

- 使用 NTT 对两个 degree-1,024 的 ℤ_p[x] 多项式相乘：比 schoolbook 快 ≥3x
  （与 `ocas-poly` 参考实现对比）。
- 稠密 `f64` 多项式求值：比解释器循环快 ≥2x（通过 `fast_polynomial` 的
  Estrin 方案）。
- F4 原型构造 cyclic-6 的 Macaulay 矩阵时，使用 `sprs` 的驻留内存 < 2 GB。
- SIMD 求值路径在 CI 中覆盖 x86-64 (AVX2)、x86-64-v4 (AVX-512) 和
  AArch64 (NEON)。

**文档**

- 新增 mdBook 章节 `performance/acceleration-libraries.md`，列出所选库、
  许可证及其 feature gate。
- 迁移说明：本版本**不**改变公共 API；所有新增内容均位于可选 feature 后
  （`pulp`、`ark-poly`、`sprs`、`faer`）。

**验收**

- 所有加速 feature 禁用时，`cargo test --workspace --exclude ocas-py` 通过。
- 每个可选库在启用对应 feature 时都能编译并通过专属 smoke test。
- `cargo deny check` 对所有新依赖通过（默认构建中无 GPL 不兼容许可证）。
- 在 `Cargo.toml` 注释中补充 feature 矩阵：`pulp`、`ark-poly`、`sprs`、
  `faer`、`fast-poly`、`verify-roots`、`verify-quadrature`。

**风险**

- `faer` 仅支持浮点运算，不能用于精确的 Gröbner 基计算；仅用于数值验证和
  求解器一致性验证。
- `sprs` 的稀疏格式与 F4 内部计划格式不同，可能需要一个薄适配层才能替换
  内部 F4 矩阵。
