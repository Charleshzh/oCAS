...
| 多项式乘法分层：Schoolbook → Karatsuba → FFT | FLINT 3 SSA；Symbolica dense mul | `ocas-poly::mul::fft` |

**性能指标**

- 对 20 次/6 次有理函数做部分分式用时 < 30 ms。
- 两个 15 次多项式的结式用时 < 20 ms（与 Symbolica 示例持平）。
- 两个 500 次 ℤ[x] 多项式乘法：比 0.11.2 Schoolbook 快 ≥5x。
