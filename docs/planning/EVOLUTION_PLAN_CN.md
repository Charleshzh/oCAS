...
- [x] 1892 题覆盖率基准运行并记录（含失败分类）：基线 5.87% → (b) 7.66%
  → (c) 见 BENCHMARK_RESULTS_CN.md；**未达 +30pp**（根因量化：复合乘积需
  因子级拆分、嵌套根式、符号系数有理后端均需更大投入；+30pp 需 Rubi 级
  规则量级）
- [x] 规则库积分在标准教科书用例上与 SymPy 输出一致（随机 100 例）
- [x] 策略调度链无回归（Risch 路径现有测试全绿）
- [x] Python/C 绑定可用
- [x]（(c) 增补验收）Weierstrass 挂死类终止（subresultant 稠密 GCD）、
  parts↔Weierstrass 循环栈溢出终止（链条目预算 256）、Wilkinson n=10
  实根隔离 10/10（精确二进分数 Sturm 求值）
