...
| Real root isolation | Sturm sequence + interval isolation + refine (univariate); exact dyadic-rational evaluation with BigInt sign decisions since 0.27 — Wilkinson n=10 isolates all 10 roots | 🟢 Complete |
...
| 18 | **Symbolic-integration breadth** (open, P0) | ⚠️ Symbolica Rubi port (7000+ rules, 72,944-problem corpus) vs oCAS Risch + 0.27 rule-table engine (families A–H) + symbolic-constant rational backend + expansion retry + trig product-to-sum + 0.27.1's seven mechanism modules (Chebyshev / trig-kernel / sqrt-quadratic / inverse-trig / denominator-power / exp-log kernels); 1892-problem coverage **16.44%** (311/1892, +6.82pp vs 0.27.0, zero regressions by per-case diff; honest record, see BENCHMARK_RESULTS_CN.md §0.27.1); +30 pp target not met (the remaining bulk is elliptic families and high-symbol-count quotients); **0.27.1 delivered, breadth gap stays open** |
