...
| **0.25.0** | 2026-08-04 | ℚ 上多素数 Gröbner 基（并行幸运素数 F5、CRT + 有理重构、无迹 p-adic Hensel 提升）、模 GCD 并行化、`Algorithm::MultiModular` 与 `Auto` 路由 |
| **0.26.0** | 2026-08-04 | u128 SWAR 打包单项式 F5 快通道、免克隆两阶段 echelon、grevlex 基准变体；修复分次单项式序的度方向反置（预存在错误基） |
| **0.27.0** | 2026-09-06 | 符号积分广度：规则表引擎（A–H 族）、符号常数有理后端、Weierstrass 线性变元、有界展开重试、三角积化和差；Rubi 1892 题覆盖率 5.87% → 9.62% |
| **0.27.1** | 2026-09-10 | 积分广度机制攻坚：Chebyshev 二项微分、三角分母幂递推、exp-log 核代换、一般二次根式引擎 + Euler III、反三角核、单三角核有理式；覆盖率 9.62% → 16.44%；修复两组错案 |
| **0.27.2** | 2026-09-11 | 已验证覆盖率口径（独立数值 oracle：5 点中心差分 + 椭圆定义积分）、阶段打点与 33 例悬挂归因、确定性阶段预算、有界展开预通道、双曲闭式族、有理导数核代换、三角相位归一、逆函数复合消去、`exp(逆函数)` 代数化、半幂前端 + 椭圆 Legendre 约化（`EllipticF/E`）；修复六组错案（已验证覆盖率 93.1%、mismatches 0、超时 33 → 13、墙钟 −22%） |
| **0.27.3** | 2026-09-12 | 0.27 线收尾：特殊函数导数表 + oracle 扩头（`Ei`/`Ei(n,z)`/`Si`/`Ci`/`Shi`/`Chi`/Fresnel）、特殊函数归约族（多项式 × `F`、`Ei(n,·)`、`F/xᵐ`、`F²`）、精确线性平方折叠 `p²+2pq+q² → (p+q)²`、半幂前端仿射变元与两支片层因子；Rubi 覆盖率 18.45% → 19.56%（新解 21、回归 0、mismatches 0、超时 13 → 12） |
...
- **正确性框架** —— 与 SymPy、SageMath、Symbolica 跨多个数学模块的交叉验证（1516 个 `#[test]`/`#[tokio::test]` 标注，截至 0.27.3）。
...
oCAS 当前版本为 **0.27.3**。核心符号引擎、多项式代数、Gröbner 基与代数几何、ODE 求解、JIT 求值、三语言绑定与正确性对比框架已达到功能完备。通往稳定 1.0 的路线见
