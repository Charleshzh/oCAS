...
│     0.27.2)                                           │
│  6. 符号常数有理后端    (integrate_rational_symbolic,  │
│     ℚ(symbols))                                       │
│  7. Risch 算法          (risch_integrate)             │
│  8. 三角→指数 + Risch + realify                       │
│  9. 特殊函数            (special_integrate)           │
│ 10. 规则表引擎          (integrate_rules, A–H 族)     │
│ 11. 二次根式引擎        (integrate_sqrt_quadratic)    │
│ 12. Chebyshev 二项微分  (integrate_binomial)          │
│ 13. exp/log 核代换      (integrate_exp_log)           │
│ 14. 反三角/反双曲       (integrate_inverse_trig)      │
│ 15. 三角积化和差        (trig_reduce_products)        │
│ 16. 三角分母幂递推      (integrate_trig_reduction)    │
│ 17. 单三角核有理式 + tan/sec                          │
│     (integrate_trig_kernel)                           │
│ 18. 有界展开重试        (expand_bounded)              │
│ 19. 半幂前端            (integrate_half_power, 0.27.2)│
│ 20. 椭圆约化            (integrate_elliptic,          │
│     EllipticF/E/Pi, 0.27.2)                           │
│ 21. 启发式方法          (heuristic_integrate)         │
│     分部积分 (LIATE)、三角替换、Weierstrass、Euler 替换│
│ 22. 未求值形式          (fallback)                    │
│     Integral(expr, var)                               │
└───────────────────────────────────────────┘
```

`OCAS_INTEGRATE_TRACE=1` 会为每个阶段打印
